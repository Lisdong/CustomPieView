...
import com.lis.pieview.PieItemBean;
...
//				canvas.drawArc(rectF, offsetAngle , currentAngle, true, mPaintBorder);
			...
			//文字
			float textSize = mPaintBorder.measureText(mDatas.get(i).name);
			Rect rect = new Rect();
			mPaintBorder.getTextBounds(mDatas.get(i).name,0,mDatas.get(i).name.length(),rect);
...
//			Log.d("角度:",arcCenterC+"");
			if (arcCenterC >=0 && arcCenterC<90){
				...
				canvas.drawText(mDatas.get(i).name,cenX+mRadius+mTextLine,arcCenterY2+textHeight,mPaintBorder);
				...
				canvas.drawText(mDatas.get(i).name,cenX-mRadius-mTextLine-textSize,arcCenterY2+textHeight,mPaintBorder);
